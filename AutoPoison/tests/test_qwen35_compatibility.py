...
class TestQwen35Contracts(unittest.TestCase):
 ...
 def test_vision_and_delta_classification(self): self.assertEqual(classify_parameter("model.visual.blocks.0.weight",P())[1],"VISION_MODULE"); self.assertEqual(classify_parameter("model.layers.0.deltanet.in_proj.weight",P())[0],"candidate"); self.assertEqual(classify_parameter("model.layers.0.linear_attn.in_proj_qkv.weight",P())[0],"candidate")
